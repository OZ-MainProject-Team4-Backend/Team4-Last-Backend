...
class FavoriteLocationModelTests(TestCase):
    ...
    def test_unique_constraint_per_user(self):
        FavoriteLocation.objects.create(
            user=self.user, city="Seoul", district="Gangnam-gu"
        )
    ...
    def test_soft_delete_allows_recreate(self):
        fl = FavoriteLocation.objects.create(
            user=self.user, city="Seoul", district="Gangnam-gu"
        )
        fl.delete()  # soft delete
        # 재생성 가능해야 함
        obj = FavoriteLocation.objects.create(
            user=self.user, city="Seoul", district="Gangnam-gu"
        )
        self.assertIsNotNone(obj)

    def test_order_auto_sorting(self):
        fl1 = FavoriteLocation.objects.create(
            user=self.user, city="Seoul", district="A", order=0
        )
        fl2 = FavoriteLocation.objects.create(
            user=self.user, city="Seoul", district="B", order=1
        )
